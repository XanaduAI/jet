...
static const std::vector<data_t> data_pow2_4x4_ba {{0,0},{4,4},{8,8},{12,12},{1,1},{5,5},{9,9},{13,13},{2,2},{6,6},{10,10},{14,14},{3,3},{7,7},{11,11},{15,15}};

static const std::vector<data_t> data_pow2_4x2x2_bac {{0,0},{1,1},{8,8},{9,9},{2,2},{3,3},{10,10},{11,11},{4,4},{5,5},{12,12},{13,13},{6,6},{7,7},{14,14},{15,15}};

static const std::vector<data_t> data_pow2_2x4x2_bac{{0,0},{1,1},{4,4},{5,5},{8,8},{9,9},{12,12},{13,13},{2,2},{3,3},{6,6},{7,7},{10,10},{11,11},{14,14},{15,15}}; 

static const std::vector<data_t> data_pow2_2x4x2_acb {{0,0},{4,4},{1,1},{5,5},{2,2},{6,6},{3,3},{7,7},{8,8},{12,12},{9,9},{13,13},{10,10},{14,14},{11,11},{15,15}};

static const std::vector<data_t> data_pow2_4x2x2_acb{{0,0},{2,2},{1,1},{3,3},{4,4},{6,6},{5,5},{7,7},{8,8},{10,10},{9,9},{11,11},{12,12},{14,14},{13,13},{15,15}};

static const std::vector<data_t> data_pow2_4x2x2_cba {{0,0},{8,8},{4,4},{12,12},{1,1},{9,9},{5,5},{13,13},{2,2},{10,10},{6,6},{14,14},{3,3},{11,11},{7,7},{15,15}};

static const std::vector<data_t> data_pow2_2x2x4_acb {{0,0},{2,2},{4,4},{6,6},{1,1},{3,3},{5,5},{7,7},{8,8},{10,10},{12,12},{14,14},{9,9},{11,11},{13,13},{15,15}};

static const std::vector<data_t> data_pow2_2x2x4_cba {{0,0},{4,4},{8,8},{12,12},{2,2},{6,6},{10,10},{14,14},{1,1},{5,5},{9,9},{13,13},{3,3},{7,7},{11,11},{15,15}};
                   ...
                   (QFlexPermuter<1024, 512>), (QFlexPermuter<2048, 1024>))
{
    ...
    SECTION("2x2x2x2"){
        ...
        SECTION("{d,b,c,a} - > {a,b,c,d}")
        {
            ...
            CHECK(permuter.Transpose(data_pow2_dbca, shape, {"d", "b", "c", "a"},
        ...
        SECTION("{c,b,a,d} - > {a,b,c,d}")
        {
            ...
            CHECK(permuter.Transpose(data_pow2_cbad, shape, {"c", "b", "a", "d"},
                                    index_expected) == data_expected);
        ...
        SECTION("{b,a,c,d} - > {a,b,c,d}")
        {
            ...
            CHECK(permuter.Transpose(data_pow2_bacd, shape, {"b", "a", "c", "d"},
                                    index_expected) == data_expected);
        ...
        SECTION("{b,a,d,c} - > {a,b,c,d}")
        {
            ...
            CHECK(permuter.Transpose(data_pow2_badc, shape, {"b", "a", "d", "c"},
                                    index_expected) == data_expected);
    ...
    SECTION("4x4"){
        ...
        SECTION("{b, a} - > {a,b}")
        {
            ...
            CHECK(permuter.Transpose(data_out, shape, {"b", "a"}, index_expected) == data_expected);

            permuter.Transpose(data_out, shape, data_out_ref,
                            {"b","a"}, index_expected);
    ...
    SECTION("2x4x2"){
        ...
        SECTION("{b, a, c} - > {a,b,c}")
        {
            ...
            CHECK(permuter.Transpose(data_out, shape, {"b", "a", "c"}, index_expected) == data_expected);

            permuter.Transpose(data_out, shape, data_out_ref,
                            {"b", "a", "c"}, index_expected);
            CHECK(data_out_ref == data_expected);
        }
        SECTION("{a, c, b} - > {a,b,c}")
        {
            ...
            CHECK(permuter.Transpose(data_out, shape, {"a", "c", "b"}, index_expected) == data_expected);

            permuter.Transpose(data_out, shape, data_out_ref,
                            {"a", "c", "b"}, index_expected);
            CHECK(data_out_ref == data_expected);
        }
        SECTION("{c, b, a} - > {a,b,c}")
        {
            ...
            CHECK(permuter.Transpose(data_out, shape, {"c", "b", "a"}, index_expected) == data_expected);
...
TEMPLATE_TEST_CASE("DefaultPermuter<>::Transpose Non power-of-2 data",
                   "[Permute]", DefaultPermuter<>)
{
    Permuter<TestType> permuter;
...
TEMPLATE_TEST_CASE("QFlexPermuter<>::Transpose Non power-of-2 data",
                   "[Permute]", QFlexPermuter<>)
{
