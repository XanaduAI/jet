...
using ComplexGpu_t = cuComplex;
using CudaTensor_t = CudaTensor<ComplexGpu_t>;
 ...
 * @return Modified name-to-task map with a defined `==` operator.
 */
TaskMap GetTaskMap(const TaskBasedContractor<CudaTensor_t> &tbc)
{
    TaskMap task_map;
    for (const auto &[name, task] : tbc.GetNameToTaskMap()) {
        task_map[name] = task.name();
 ...
 * @return Modified name-to-tensor map with a defined `==` operator.
 */
TensorMap GetTensorMap(const TaskBasedContractor<CudaTensor_t> &tbc)
{
    TensorMap tensor_map;
    for (const auto &[name, tensor_ptr] : tbc.GetNameToTensorMap()) {
        std::vector<complex_t> tensor_data;
        if (tensor_ptr.get() != nullptr) {
            tensor_data = tensor_ptr->GetHostDataVector();
        }
        tensor_map.emplace(name, tensor_data);
    }
    return tensor_map;
...
TEST_CASE("TaskBasedContractor<CudaTensor>::AddContractionTasks()",
          "[TaskBasedContractor]")
{
    TaskBasedContractor<CudaTensor_t> tbc;

    SECTION("Tensor network is empty")
    {
        const TensorNetwork<CudaTensor_t> tn;
        const PathInfo path_info;
    ...
    SECTION("Paths have shared contractions")
    {
        ...
        const auto tensor_2 = MakeCudaTensor({"A0"}, {2});
        const auto tensor_3 = MakeCudaTensor({"B1"}, {3});
...
TEST_CASE("TaskBasedContractor<CudaTensor>::Contract()",
          "[TaskBasedContractor]")
{
    TaskBasedContractor<CudaTensor_t> tbc;
    ...
    SECTION("Taskflow has a single contraction task")
    {
        const auto tensor_1 = MakeCudaTensor({"A0"}, {2});
        const auto tensor_2 = MakeCudaTensor({"A0", "B1"}, {2, 3});
    ...
    SECTION("Taskflow has multiple result tasks")
    {
        ...
        const auto tensor_3 = MakeCudaTensor({"B1"}, {4});
        const auto tensor_4 = MakeCudaTensor({"B1"}, {4});
...
TEST_CASE("TaskBasedContractor<CudaTensor>::AddDeletionTasks()",
          "[TaskBasedContractor]")
{
    TaskBasedContractor<CudaTensor_t> tbc;
...
TEST_CASE("TaskBasedContractor<CudaTensor>::AddReductionTask()",
          "[TaskBasedContractor]")
{
    TaskBasedContractor<CudaTensor_t> tbc;
