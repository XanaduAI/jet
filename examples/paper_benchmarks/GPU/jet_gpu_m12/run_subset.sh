...
OMP_NUM_THREADS=1 ./jet_cuda_m12_subset ../../data_files/m12.json 1 9 > /dev/null
    ...
    for p in 1; do
        for th in 1; do
            for i in $(seq 1 $num_runs); do
                echo "### RUN=${i} ###" >> jet_cuda_subset_omp${th}_pthread${p}_slice${sl}.out;
                OMP_NUM_THREADS=${th} ./jet_cuda_m12_subset ../../data_files/m12.json ${p} ${sl} >> jet_cuda_subset_omp${th}_pthread${p}_slice${sl}.out
            done
        done
    done
done
...
# Format data as CSV
touch jet_subset_data.csv

t_samples=""
for i in $(seq 0 $(($num_runs-1))); do t_samples+="t${i},"; done

echo "OMP,PTHREAD,SLICE,${t_samples}" | sed 's/.$//' >> jet_subset_data.csv
for i in $(ls -trah | grep jet_cuda_subset_omp[1-9]);
do
    ...
    dat_line+=$(echo "");
    echo $dat_line >> jet_subset_data.csv
done
