...
OMP_NUM_THREADS=1 ./jet_cuda_m12_full ../../data_files/m12.json 1 9 > /dev/null
    ...
    for p in 1; do
        for th in 1; do
            for i in $(seq 1 $num_runs); do
                ...
                OMP_NUM_THREADS=${th} ./jet_cuda_m12_full ../../data_files/m12.json ${p} ${sl} >> jet_cuda_full_omp${th}_pthread${p}_slice${sl}.out
            done
        done
    done
done
